...
This article demonstrates every `edgelist()`, `nodelist()`, `as_igraph()`, and `as_tbl_graph()` method in the package. For a gentler introduction, see `vignette("networkformat")`.
...
```{r}
edgelist(courses, source_cols = crosslist, target_cols = course,
...
```{r}
edgelist(courses,
         source_cols = c(prereq, crosslist),
         target_cols = course,
...
### as_igraph / as_tbl_graph availability

| Input | as_igraph | as_tbl_graph | `treenum` param |
|-------|-----------|--------------|-----------------|
| vector | -- | -- | -- |
| data.frame | -- | -- | -- |
| tree | yes | yes | no |
| rpart | yes | yes | no |
| randomForest | yes | yes | yes (default `NULL`) |
| gbm | yes | yes | yes (default `NULL`) |
| xgb.Booster | yes | yes | yes (default `NULL`) |
