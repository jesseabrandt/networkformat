...
For vectors and data frames, use `edgelist()` and `nodelist()` to build
the edge/node data, then pass them to igraph or tidygraph. If you want
to work with the raw edgelists and nodelists directly --- filtering edges,
computing graph statistics, or building custom igraph objects --- see
...
`as_tbl_graph(rf)` combines all trees into a single graph
...
All three trees are visible in a single plot. Internal nodes are colored by
split variable; leaf nodes are colored by predicted class.
