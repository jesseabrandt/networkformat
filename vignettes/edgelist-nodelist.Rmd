...
| `rpart` | --- | single graph |
| `xgb.Booster` | `1` (default) | single graph (one tree) |
| `xgb.Booster` | `NULL` | single graph (all trees, globally unique string IDs) |
| `gbm` | `1` (default) | single graph (one tree) |
| `gbm` | `NULL` | single graph (all trees as disconnected components) |
