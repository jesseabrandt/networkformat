...
g_forest <- graph_from_data_frame(rf_edges_all[, c("from", "to", "treenum")],
                                   directed = TRUE)
...
tree_depths <- rf_edges_all %>%
  ...
  summarise(
    n_edges = n(),
    n_nodes = length(unique(c(from, to))),
    max_depth = max(from)  # Approximation
  )
...
data.frame(
  ...
  n_nodes = c(
    ...
    length(unique(c(edges_rf$from, edges_rf$to))) / 10
  )
)
```
