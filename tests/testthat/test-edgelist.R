...
test_that("edgelist on randomForest produces data frame", {
  # skip_if_not_installed("randomForest")
  library(randomForest)#simplify?
  rf <- randomForest(mpg ~ ., data = mtcars)
  edgelist <- edgelist(rf)
  expect_s3_class(edgelist, "data.frame")
})

test_that("edgelist data.frame method returns df", {
  ...
  edgelist <- edgelist.data.frame(df)
  # expect_null(result)
  expect_s3_class(edgelist, "data.frame")
})
